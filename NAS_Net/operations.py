...
class Nlinear(nn.Module):
    def __init__(self, input_lengh, output_length, c_in):
        ...
        if self.individual:
            ...
            for i in range(self.channels):
                self.Nlinear_Linear.append(nn.Linear(self.seq_len, self.pred_len))
        else:
            self.Nlinear_Linear = nn.Linear(self.seq_len, self.pred_len)
...
class DCCLayer(nn.Module):
    ...
    def __init__(self, c_in, c_out, kernel_size, dilation=1):
        ...
        self.dcc_filter_conv = CausalConv2d(c_in, c_out, kernel_size, dilation=dilation)
        self.dcc_gate_conv = CausalConv2d(c_in, c_out, kernel_size, dilation=dilation)
...
class CNN(nn.Module):
    ...
    def __init__(self, c_in, c_out, kernel_size, stride=1, dilation=1):
        ...
        # self.relu = nn.ReLU()
        self.cnn_filter_conv = CausalConv2d(c_in, c_out, kernel_size, stride, dilation=dilation)
...
class dilated_inception(nn.Module):
    def __init__(self, cin, cout, dilation_factor=2):  # dilation=2时速度很慢？？？
        super(dilated_inception, self).__init__()
        self.tconv = nn.ModuleList()
        self.kernel_set = [2, 3, 6, 7]
        cout = int(cout / len(self.kernel_set))
        for kern in self.kernel_set:
            self.tconv.append(CausalConv2d(cin, cout, (1, kern), dilation=dilation_factor))
...
class dilated_inception_layer(nn.Module):
    ...
    def __init__(self, cin, cout, dilation_factor=1):
        ...
        self.inception_filter_conv = dilated_inception(cin, cout, dilation_factor)
        self.inception_gate_conv = dilated_inception(cin, cout, dilation_factor)
...
class GRU(nn.Module):
    def __init__(self, c_in, c_out):
        ...
        self.gru = nn.GRU(c_in, c_out, bidirectional=True, batch_first=True)
        self.c_out = c_out
...
class LSTM(nn.Module):
    def __init__(self, c_in, c_out, ):
        ...
        # 双向lstm比单向性能好点
        self.lstm = nn.LSTM(c_in, c_out, bidirectional=True, batch_first=True)
        self.c_out = c_out
...
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads,
                 d_keys=None, d_values=None, mix=False, convformer=False):
        ...
        if convformer:
            ...
            self.value_projection = SepConv1d(d_model, d_model, 1)
        else:
...
class InformerLayer(nn.Module):
    def __init__(self, d_model, d_ff=32, dropout=0., n_heads=4, activation="relu",
                 output_attention=False, informer=True, convformer=False):
...
class DiffusionConvLayer(nn.Module):
    ...
    def __init__(self, K, supports, nodevec1, nodevec2, c_in, c_out, dropout=False):
        ...
        self.diffusionconv_mlp = linear(c_in, c_out).to(DEVICE)  # 7 * 32 * 32
        self.c_out = c_out
        self.K = K
...
class mixprop(nn.Module):
    def __init__(self, c_in, c_out, gdep=2, dropout=0., alpha=0.05):
        super(mixprop, self).__init__()
        self.nconv = nconv_mix()
        self.mlp = linear((gdep + 1) * c_in, c_out).to(DEVICE)
...
class Bidirection_mixprop(nn.Module):
    ...
    def __init__(self, c_in, c_out, nodevec1, nodevec2, gdep=2, dropout=0., alpha=0.05):
        super(Bidirection_mixprop, self).__init__()
        self.mixprop1 = mixprop(c_in, c_out, gdep, dropout, alpha)
        self.mixprop2 = mixprop(c_in, c_out, gdep, dropout, alpha)
        self.nodevec1 = nodevec1
        self.nodevec2 = nodevec2
